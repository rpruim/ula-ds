...
for scale in [1, 8, 10, 12]:
      beginfigure("projection-line-" + str(scale), width+2*margin, height+2*margin)
      ...
      if scale > 0.2: 
            ...
            Label(r"${k \boldsymbol x}$", 0.8 * proj, alignment="lt", offset=[4,-4], color = lightblue).draw()
            Label(r"${\boldsymbol y} - k {\boldsymbol x}$", kx + 0.5 * (y - kx), color = lightred, alignment="lb", offset=[4,0]).draw()

      Vector(x, fillcolor=black).fill()
      Vector(y, fillcolor=black).fill()

      Label(r"${\boldsymbol y}$", y * 0.75, alignment="rb", offset=[-4,4]).draw()
      Label(r"${\boldsymbol x}$", x * 0.75, alignment="lt", offset=[4,-4]).draw()
